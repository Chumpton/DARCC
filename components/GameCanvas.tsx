...
export const GameCanvas: React.FC<GameCanvasProps> = ({ onUiUpdate, gameActionsRef, isPaused }) => {
  ...
  const createImpactPuff = (pos: Vector2, spellType: SpellType) => {
      ...
      for(let i=0; i<3; i++) {
          stateRef.current.visualEffects.push({
              ...
              type: 'impact_puff',
              pos: { 
                  x: pos.x + (Math.random() - 1.2) * 1.2, 
                  y: pos.y + (Math.random() - 1.2) * 1.2 
              },
              life: 15, // Short life
    ...
    for (let tx = minX; tx <= maxX; tx++) {
        for (let ty = minY; ty <= maxY; ty++) {
            ...
            if (tile.hasTree) {
                ...
                const treeState = stateRef.current.trees[`${tx},${ty}`];
    ...
    state.projectiles.forEach(p => {
      ...
      if (p.spellType !== SpellType.BOMB && !isPositionValid(nextX, nextY, p.radius)) {
          ...
          if (p.spellType === SpellType.FIRE) {
              ...
              const minX = Math.floor(nextX - checkRadius);
              const maxX = Math.ceil(nextX + checkRadius);
              const minY = Math.floor(nextY - checkRadius);
              const maxY = Math.ceil(nextY + checkRadius);
              ...
              for (let tx = minX; tx <= maxX; tx++) {
                  for (let ty = minY; ty <= maxY; ty++) {
                      ...
                      if (tile.hasTree) {
                          const treePos = { x: tx + 0.5, y: ty + 0.5 };
                          const dist = getDistance({x: nextX, y: nextY}, treePos);
                          if (dist < 1.0) {
                              // Ignite Tree
                              const key = `${tx},${ty}`;
                              if (!state.trees[key]) {
                                  state.trees[key] = { pos: treePos, burnTimer: 540, isBurnt: false };
                                  createImpactPuff(treePos, SpellType.FIRE); // Fire effect
                              }
                              hitTree = true;
                          }
                      }
               ...
               if (p.explosionRadius && p.explosionRadius > 0 && !p.isShrapnel && !p.isEnemy) {
                   createExplosion(p.pos, p.explosionRadius, p.damage, SPELL_CONFIG.FIRE.color);
               } else if (state.player.level >= LEVEL_5_UNLOCK && !p.isShrapnel && !p.isEnemy) {
  ...
  const render = () => {
      ...
      const { player, enemies, projectiles, loot, texts, visualEffects, trees } = stateRef.current;
      ...
      for (let x = startX; x <= endX; x++) {
          for (let y = startY; y <= endY; y++) {
              ...
              if (tile.hasTree) {
                  const key = `${x},${y}`;
                  const treeState = trees[key];
                  
                  // Skip rendering if fully burnt
                  if (!treeState || !treeState.isBurnt) {
                       ...
                       renderList.push({
                          ...
                          pos: worldPos,
                          data: { 
                              variant: tile.treeVariant,
                              isBurning: treeState && treeState.burnTimer > 0 
                          }
                      });
                  }
      ...
      renderList.forEach(item => {
          ...
          if (item.type === 'tree') {
               ...
               // Render Fire Overlay if burning
               if (item.data.isBurning) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Spells Mapped to Q W E R T Y U I O
      if (k === 'q') castSpell(mouse, SpellType.FIRE);
      if (k === 'w') castSpell(mouse, SpellType.ICE);
      if (k === 'e') castSpell(mouse, SpellType.LIGHTNING);
      if (k === 'r') castSpell(mouse, SpellType.WIND);
      if (k === 't') castSpell(mouse, SpellType.ARCANE_EXPLOSION);
      if (k === 'y') castSpell(mouse, SpellType.TELEPORT);
      if (k === 'u') castSpell(mouse, SpellType.BOMB);
      if (k === 'i') {
          const p = stateRef.current.player;
      ...
      if (k === 'o') castSpell(mouse, SpellType.EARTH);

      // Potions Mapped to 1 2 3
      if (k === '1') consumePotion('health');
      if (k === '2') consumePotion('mana');
      if (k === '3') consumePotion('speed');
    };
